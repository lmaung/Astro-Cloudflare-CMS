...
import { routeForPage } from './lib/content-snapshot';
...
describe('discoverability endpoints', () => {
  ...
  it('places protected pages under the authenticated route family', () => {
    expect(routeForPage({ slug: 'news', access: { readRoles: ['public'] } })).toBe('/news');
    expect(routeForPage({ slug: 'minutes', access: { readRoles: ['member'] } })).toBe('/members/minutes');
  });

  it('escapes sitemap locations', () => {
    const sitemap = renderSitemap(['https://example.com/search?q=one&sort=two']);
    expect(sitemap).toContain('q=one&amp;sort=two');
  });

  it('resolves an exact redirect and preserves non-conflicting query values', () => {
    const redirect = resolveRedirect(new URL('https://example.com/old/?campaign=summer'), {
      redirects: [
        {
          id: 'old',
          from: '/old',
