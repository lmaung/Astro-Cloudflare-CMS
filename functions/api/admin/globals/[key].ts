...
export const onRequest: PagesHandler<AdminEnv> = async ({ request, env, params }) => {
  try {
    ...
    const key = params.key; if (key !== 'site-settings' && key !== 'navigation' && key !== 'reusable-blocks' && key !== 'media-library' && key !== 'redirects') return json({ code: 'unsafe_path', message: 'Unsupported global content key.' }, 400);
