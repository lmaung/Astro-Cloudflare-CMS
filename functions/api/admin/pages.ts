...
export const onRequest: PagesHandler<AdminEnv> = async ({ request, env }) => {
  try {
    ...
    if (request.method === 'POST') {
      ...
      const body = await request.json() as { data?: unknown; expectedRevision?: unknown; changeId?: unknown };
      if (typeof body.expectedRevision !== 'string' || typeof body.changeId !== 'string') return json({ code: 'invalid_content', message: 'Expected revision and change identifier are required.' }, 400);
      return json(await createPageDirect(client, config, { data: body.data, expectedRevision: body.expectedRevision, changeId: body.changeId }), 201);
    }
    ...
    return json({ code: 'unavailable', message: 'The page collection is unavailable. No content was changed.' }, 503);
  }
};
