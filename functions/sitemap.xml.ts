...
export const onRequestGet: PagesHandler<AdminEnv> = async ({ env }) => {
  try {
    ...
    const published = summaries.data.filter((page) => page.status === 'published');
    const pages = await Promise.all(published.map((page) => readPage(client, config, page.slug)));
    const urls = pages
      .filter((page) => !page.data.seo.noIndex && page.data.access.readRoles.includes('public'))
      .map((page) => new URL(page.data.slug === 'home' ? '/' : `/${page.data.slug}`, settings.data.siteUrl).href);
