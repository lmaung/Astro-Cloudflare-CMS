...
export class ContentRequestError extends Error {
  constructor(
    readonly code:
      'change_conflict' | 'invalid_content' | 'not_found' | 'stale_revision' | 'unsafe_path' | 'unavailable',
    message: string,
  ) {
    super(message);
  }
...
async function assertPageIsNotInNavigation(client: GitHubClient, config: AdminConfig, slug: string) {
  try {
    const file = await client.request<GitHubContent>(
      `/contents/globals/navigation.json?ref=${encodeURIComponent(config.contentBranch)}`,
    );
    const navigation = navigationSchema.parse(JSON.parse(decodeBase64(file.content)));
    const routes = [hrefForSlug(slug), `/members/${slug}`];
    if (navigation.primary.some((item) => routes.includes(item.href.replace(/\/$/, '') || '/'))) {
...
async function assertPageHasNoReusableDependencies(client: GitHubClient, config: AdminConfig, slug: string) {
  try {
    const files = await client.request<GitHubContent[]>(
      `/contents/reusables?ref=${encodeURIComponent(config.contentBranch)}`,
    );
    for (const file of files.filter((item) => item.type === 'file' && item.name?.endsWith('.json'))) {
      const reusable = await client.request<GitHubContent>(
        `/contents/reusables/${encodeURIComponent(file.name!)}?ref=${encodeURIComponent(config.contentBranch)}`,
      );
      const source = decodeBase64(reusable.content);
      if (source.includes(`\"/${slug}\"`) || source.includes(`\"/${slug}/\"`)) {
        throw new ContentRequestError(
          'change_conflict',
          `Reusable content “${file.name}” still links to this page. Remove that dependency before deleting it.`,
        );
      }
    }
  } catch (error) {
    ...
    throw new ContentRequestError(
      'unavailable',
      'Reusable-content dependencies could not be checked. The page was not deleted.',
    );
  }
}
...
async function validateReusableInstances(client: GitHubClient, config: AdminConfig, page: PageDocument) {
  ...
  try {
    const file = await client.request<GitHubContent>(
      `/contents/globals/reusable-blocks.json?ref=${encodeURIComponent(config.contentBranch)}`,
    );
    library = reusableLibrarySchema.parse(JSON.parse(decodeBase64(file.content)));
  } catch {
    throw new ContentRequestError(
      'invalid_content',
      'Reusable block references cannot be resolved. Save the reusable library first.',
    );
  }
  for (const block of linked) {
    ...
    if (!source || source.type !== block.type)
      throw new ContentRequestError(
        'invalid_content',
        `Reusable source “${block.reusable?.sourceId}” is missing or has a different block type.`,
      );
    const invalidField = Object.keys(block.reusable?.overrides ?? {}).find(
      (field) => !source.refinableFields.includes(field as never),
    );
    if (invalidField)
      throw new ContentRequestError(
        'invalid_content',
        `Field “${invalidField}” is not refinable for reusable source “${source.name}”.`,
      );
    try {
      validateBlock(block.type, resolveReusableBlock(block, library).content);
    } catch (error) {
      throw new ContentRequestError('invalid_content', `Reusable block validation failed: ${String(error)}`);
    }
  }
}
...
async function readPageAtRef(client: GitHubClient, slug: string, ref: string) {
  try {
    ...
    if (file.encoding !== 'base64')
      throw new ContentRequestError('unavailable', 'GitHub returned unsupported content encoding.');
    return {
      data: validatePage(JSON.parse(decodeBase64(file.content)), slug),
      revision: file.sha,
...
export async function createPageDirect(
  ...
  try {
    candidate = pageSchema.parse(input.data);
  } catch (error) {
    throw new ContentRequestError('invalid_content', `Content validation failed: ${String(error)}`);
  }
  const page = validatePage(candidate, candidate.slug);
  await validateReusableInstances(client, config, page);
  if (page.slug === 'admin' || page.slug === 'api')
    throw new ContentRequestError('unsafe_path', 'This slug is reserved by the platform.');
  const branch = await client.request<{ object: { sha: string } }>(
    `/git/ref/heads/${encodeURIComponent(config.contentBranch)}`,
  );
  if (branch.object.sha !== input.expectedRevision)
    throw new ContentRequestError(
      'stale_revision',
      'The page collection changed after it was loaded. Reload before creating a page.',
    );
  try {
  ...
  try {
    ...
    return {
      data: page,
      revision: saved.revisions[pagePath(page.slug)]!,
      collectionRevision: saved.commit,
      mode: 'remote' as const,
      submission: { kind: 'direct_save' as const },
    };
  } catch (error) {
    if (error instanceof ContentRequestError) throw error;
    throw new ContentRequestError(
      'unavailable',
      'GitHub could not create the page. No frontend deployment was triggered.',
    );
  }
}

export async function savePageDirect(
  client: GitHubClient,
  config: AdminConfig,
  slug: string,
  input: { data: unknown; expectedRevision: string; changeId: string },
  ...
  await validateReusableInstances(client, config, page);
  if (slug === 'home' && page.status === 'archived')
    throw new ContentRequestError('invalid_content', 'The home page cannot be archived.');
  if (slug === 'home' && !page.access.readRoles.includes('public'))
    throw new ContentRequestError('invalid_content', 'The home page must remain publicly readable.');
  const current = await readPage(client, config, slug);
  if (current.revision !== input.expectedRevision)
    throw new ContentRequestError(
      'stale_revision',
      'Published content changed after this editor was loaded. Reload before saving.',
    );
  if (current.data.status !== 'archived' && page.status === 'archived')
  ...
  try {
    const saved = await commitFilesToMain(
      client,
      config.contentBranch,
      `${page.status === 'archived' ? 'Archive' : 'Update'} ${page.title}`,
      [
    ...
    return {
      data: page,
      revision: saved.revisions[pagePath(slug)]!,
      mode: 'remote' as const,
      submission: { kind: 'direct_save' as const },
    };
  } catch (error) {
    ...
    throw new ContentRequestError(
      'unavailable',
      'GitHub could not save the content. No frontend deployment was triggered.',
    );
  }
}

export async function deletePageDirect(
  client: GitHubClient,
  config: AdminConfig,
  slug: string,
  input: { expectedRevision: string; confirmation: string },
) {
  ...
  if (current.revision !== input.expectedRevision)
    throw new ContentRequestError(
      'stale_revision',
      'The page changed after this editor was loaded. Reload before deleting it.',
    );
  if (current.data.status !== 'archived')
    throw new ContentRequestError('invalid_content', 'Archive this page before permanently deleting it.');
  if (input.confirmation.trim() !== current.data.slug && input.confirmation.trim() !== current.data.title) {
    throw new ContentRequestError(
      'invalid_content',
      `Type “${current.data.slug}” or the exact page title to confirm permanent deletion.`,
    );
  }
  await assertPageIsNotInNavigation(client, config, slug);
