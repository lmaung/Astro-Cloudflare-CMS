...
describe('direct page publishing', () => {
  it('validates and atomically saves page plus proof directly to main', async () => {
    ...
    const result = await savePageDirect(client, config, 'home', {
      data: page,
  ...
  it('rejects a stale page before creating Git objects', async () => {
    ...
    await expect(
      savePageDirect({ request } as GitHubClient, config, 'home', {
        data: page,
      ...
      code: 'stale_revision',
    } satisfies Partial<ContentRequestError>);
  ...
  it('protects the home page from archival', async () => {
    ...
    await expect(
      savePageDirect(client, config, 'home', {
  ...
  it('blocks archival while navigation still references the page', async () => {
    ...
    const request = vi.fn(async (path: string) => {
      if (path === '/contents/pages/about.json?ref=main')
        return {
          content: encoded(about),
          encoding: 'base64',
          sha: 'about-old',
        };
      if (path === '/contents/globals/navigation.json?ref=main')
        return {
          content: btoa(JSON.stringify({ primary: [{ label: 'About', href: '/about' }] })),
          encoding: 'base64',
          sha: 'nav',
        };
      throw new Error(`Unexpected request: ${path}`);
    ...
    await expect(
      savePageDirect({ request } as GitHubClient, config, 'about', {
        data: { ...about, status: 'archived' },
        expectedRevision: 'about-old',
        changeId,
      }),
  ...
  it('creates a new page with an atomic page and validation commit', async () => {
    ...
    const result = await createPageDirect({ request } as GitHubClient, config, {
      data: about,
      expectedRevision: 'commit-main',
      changeId,
    });
    expect(result.data.slug).toBe('about');
    expect(result.collectionRevision).toBe('commit-new');
    const tree = JSON.parse(String(calls.find((call) => call.path === '/git/trees')?.init?.body)).tree;
    expect(tree.map((entry: { path: string }) => entry.path)).toEqual([
      'pages/about.json',
      '_validation/pages/about.json',
    ]);
  });
  ...
  it('permanently deletes an archived page and its validation artifact atomically', async () => {
    ...
    const request = vi.fn(async (path: string, init?: RequestInit) => {
      ...
      if (path === '/contents/pages/about.json?ref=main')
        return {
          content: encoded(archived),
          encoding: 'base64',
          sha: 'about-old',
        };
      if (path === '/contents/globals/navigation.json?ref=main')
        return {
          content: btoa(JSON.stringify({ primary: [{ label: 'Home', href: '/' }] })),
          encoding: 'base64',
          sha: 'nav',
        };
    ...
    const result = await deletePageDirect({ request } as GitHubClient, config, 'about', {
      expectedRevision: 'about-old',
      confirmation: 'about',
    });
    expect(result).toMatchObject({
      deleted: true,
      slug: 'about',
      collectionRevision: 'commit-new',
    });
    const tree = JSON.parse(String(calls.find((call) => call.path === '/git/trees')?.init?.body)).tree;
    expect(tree).toEqual([
      { path: 'pages/about.json', mode: '100644', type: 'blob', sha: null },
      {
        path: '_validation/pages/about.json',
        mode: '100644',
        type: 'blob',
        sha: null,
      },
  ...
  it('rejects permanent deletion until the page is archived and confirmed', async () => {
    const { client } = directSaveClient();
    await expect(
      deletePageDirect(client, config, 'home', {
        expectedRevision: 'blob-old',
        confirmation: 'home',
      }),
    ).rejects.toMatchObject({ code: 'invalid_content' });
    ...
    await expect(
      deletePageDirect({ request } as GitHubClient, config, 'about', {
        expectedRevision: 'about-old',
        confirmation: 'about',
      }),
    ).rejects.toMatchObject({ code: 'invalid_content' });
