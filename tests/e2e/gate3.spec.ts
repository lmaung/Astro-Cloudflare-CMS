...
async function mockAdmin(page: Page) {
  ...
  await page.route('**/api/admin/globals/*', async (route) => {
    ...
    const data = key === 'navigation' ? { primary: [{ label: 'Home', href: '/' }] }
      : key === 'reusable-blocks' ? { blocks: [{ id: 'shared-hero', name: 'Shared hero', description: 'Reusable introduction', type: 'core/hero', content: { eyebrow: 'Shared', heading: 'Shared heading', body: 'Shared body' }, refinableFields: ['heading'] }] }
      : key === 'media-library' ? { assets: [] }
      : { siteName: 'Example', tagline: 'Example site', defaultSeo: { titleSuffix: 'Example', description: 'Example description' }, footer: { copyright: '© Example' } };
...
test('reusable blocks can be linked, refined, and detached', async ({ page }) => {
  await mockAdmin(page);
  await page.goto('/admin/');
  await expect(page.getByText('Published page loaded. Saves publish immediately without redeploying.')).toBeVisible();
...
test('Gate 4 library editors expose usable, axe-clean controls', async ({ page }) => {
  await mockAdmin(page);
  await page.goto('/admin/');
  await expect(page.getByText('Published page loaded. Saves publish immediately without redeploying.')).toBeVisible();
  await page.getByRole('button', { name: 'Reusable blocks', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Reusable blocks', exact: true })).toBeVisible();
...
test('runtime refresh renders registered layouts and the detailed footer without deployment', async ({ page }) => {
  await page.route('**/api/content/snapshot*', async (route) => route.fulfill({ json: {
    page: { ...home, blocks: [{ id: 'grid', type: 'core/content-grid', status: 'active', content: { heading: 'Runtime services', layout: 'three-column', alignment: 'start', surface: 'subtle', spacing: 'default', items: [{ id: 'one', heading: 'One', body: 'First' }, { id: 'two', heading: 'Two', body: 'Second' }, { id: 'three', heading: 'Three', body: 'Third' }] } }] },
    settings: { siteName: 'Runtime site', tagline: 'Current content', defaultSeo: { titleSuffix: 'Runtime', description: 'Runtime content' }, footer: { copyright: '© Runtime', summary: 'Detailed footer summary', columns: [{ id: 'company', title: 'Company', links: [{ label: 'About us', href: '/about' }] }], socialLinks: [{ label: 'LinkedIn', href: 'https://www.linkedin.com/' }], legalLinks: [], supportingImages: [], newsletter: { enabled: true, heading: 'Updates', description: 'News from us', actionLabel: 'Subscribe', actionHref: '/subscribe', privacyNote: 'No spam.' }, appearance: { variant: 'dark', overlay: 'medium' } } },
