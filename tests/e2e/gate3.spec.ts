  ...
  await page.route('**/api/admin/authorization', async (route) => {
    const data = {
      roles: [
        ...
        { key: 'member', name: 'Member', description: 'Members', system: true },
        { key: 'editor', name: 'Editor', description: 'Editors', system: true },
        {
          key: 'page-editor',
          name: 'Page editor',
          description: 'Page writers',
          system: true,
        },
        {
          key: 'admin',
    ...
    await route.fulfill({
      json: { data, revision: 'authorization-blob', currentUser: { email: 'admin@example.com' } },
    });
  });
}
...
test('admin page lifecycle is keyboard-operable and axe-clean', async ({ page }) => {
  ...
  await expect(
    page.getByText('Page created and published. Add it to Navigation when it should appear in the menu.'),
  ).toBeVisible();
...
test('page permissions and private role assignments are accessible', async ({ page }) => {
  await mockAdmin(page);
  await page.goto('/admin/');
  await expect(page.getByRole('group', { name: 'Who can read this page?' })).toBeVisible();
  await expect(page.getByRole('group', { name: 'Who can edit this page?' })).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'User assignments', exact: true })).toBeVisible();
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
});
