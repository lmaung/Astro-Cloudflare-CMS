...
import { authorizationDirectorySchema } from '../domain/authorization';
...
export class LocalFilesystemProvider implements ContentReader, ContentWriter {
  ...
  async readGlobal(key: 'site-settings' | 'navigation' | 'reusable-blocks' | 'media-library' | 'redirects' | 'authorization'): Promise<Versioned<unknown>> {
    await this.verifyManifest();
    let source: string;
    try { source = await readFile(path.join(this.root, 'globals', `${key}.json`), 'utf8'); }
    catch (error) { if ((key === 'reusable-blocks' || key === 'media-library' || key === 'redirects' || key === 'authorization') && (error as NodeJS.ErrnoException).code === 'ENOENT') return { data: key === 'reusable-blocks' ? { blocks: [] } : key === 'media-library' ? { assets: [] } : key === 'authorization' ? { roles: [], users: [] } : { redirects: [] }, revision: revisionFor('') }; throw error; }
    const schema = key === 'site-settings' ? siteSettingsSchema : key === 'navigation' ? navigationSchema : key === 'reusable-blocks' ? reusableLibrarySchema : key === 'media-library' ? mediaLibrarySchema : key === 'authorization' ? authorizationDirectorySchema : redirectsSchema;
    return { data: schema.parse(JSON.parse(source)), revision: revisionFor(source) };
  }

  async writeGlobal(key: 'site-settings' | 'navigation' | 'reusable-blocks' | 'media-library' | 'redirects' | 'authorization', data: unknown, expectedRevision: string): Promise<Versioned<unknown>> {
