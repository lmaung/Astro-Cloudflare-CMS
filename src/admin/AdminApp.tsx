...
export default function AdminApp() {
  ...
  function updateBlocks(blocks: BlockEnvelope[], nextSelectedId = selectedId) {
    if (page) markPage({ ...page, blocks }, nextSelectedId);
  }
  ...
  function attachReusable(sourceId: string) {
    ...
    blocks[selectedIndex] = {
      ...
      reusable: { sourceId, overrides: {} },
    };
    updateBlocks(blocks);
  }
  function detachReusable() {
    if (!page || selectedIndex < 0 || !selectedBlock?.reusable) return;
    const blocks = [...page.blocks];
    const resolved = resolveReusableBlock(selectedBlock, reusableLibrary);
    blocks[selectedIndex] = {
      id: resolved.id,
      type: resolved.type,
      status: resolved.status,
      content: structuredClone(resolved.content),
    };
    updateBlocks(blocks);
  }
  ...
  async function submitNewPage(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const slug = newSlug || slugFromTitle(newTitle);
    if (!slug) return;
  ...
  async function toggleArchived() {
    ...
    if (
      archiving &&
      !window.confirm(
        `Archive “${page.title}”? Its public URL will stop displaying the page. Remove any navigation links to it separately.`,
      )
    )
      return;
      ...
      {globalDefinition ? (
        ...
        <>
          ...
          <div className="page-manager">
            <aside className="page-list-panel" aria-labelledby="pages-heading">
              <div className="panel-heading">
                <div>
                  <p className="admin-kicker">Website</p>
                  <h1 id="pages-heading">Pages</h1>
                </div>
                <button
                  className="button button--secondary"
                  type="button"
              ...
              {creating && (
                <form className="new-page-form" onSubmit={submitNewPage}>
                  <label>
                    Page title
                    <input
                      required
            ...
            {page ? (
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          markPage({
                            ...
                            seo: {
                              ...page.seo,
                              socialImage: event.target.value,
                            },
                          })
                        }
                      />
                      <small>Use a site-relative path or HTTPS URL.</small>
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          markPage({
                            ...
                            seo: {
                              ...page.seo,
                              socialImageAlt: event.target.value,
                            },
                          })
                        }
                      />
                  ...
                  >
                    {selectedBlock && selectedDefinition ? (
                      <>
                        ...
                        <div className="reusable-controls">
                          {selectedBlock.reusable ? (
                            <>
                              <div>
                                <strong>
                                  Linked to{" "}
                      ...
                      <div className="admin-empty">
                        <h2 id="editor-heading">No block selected</h2>
                        <p>Add or select a block to edit its content.</p>
                      </div>
                    )}
                  </section>
                </div>
              ...
              <div className="admin-empty">
                ...
                <p>Create or select a page to continue.</p>
              </div>
            )}
          </div>
        </>
