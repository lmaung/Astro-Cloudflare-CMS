...
export function GlobalEditor({ definition }: { definition: { key: string; title: string; description: string; schema: RJSFSchema } }) {
  ...
  async function save() { const result = await saveGlobal(definition.key, data, revision, crypto.randomUUID()); setRevision(result.revision); setDirty(false); setMessage(result.mode === 'remote' ? 'Content pull request created. No frontend deployment was triggered.' : 'Saved locally without a Git commit.'); }
